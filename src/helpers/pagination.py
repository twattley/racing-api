...
def build_pagination_links(url: URL, limit: int, page: int, has_more_data: bool) -> PaginationLinks:
    prev_url = url.include_query_params(page=page - 1, limit=limit) if page > 1 else None
    next_url = url.include_query_params(page=page + 1, limit=limit) if has_more_data else None
